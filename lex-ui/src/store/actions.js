...
export default {
  ...
  uploadPicture(context, imageData) {
    context.dispatch('pushMessage', {
      type: 'human',
    ...
    return new Promise((resolve, reject) => {
      axios({
        method: 'POST',
        url: '{YOUR_API_GATEWAY_TO_UPLOAD_FILE_LAMBDA}',
        crossDomain: true,
        xDomainRequest: true,
        xDomain: true,
        headers: {
        ...
        .then((res) => {
          const session = context.state.config.lex.sessionAttributes;
          session.lastUploadedURL = res.data.body.url;
  ...
  postTextMessage(context, message) {
    return context.dispatch('interruptSpeechConversation')
      ...
      .then(() => context.dispatch('lexPostText', message))
      .then((response) => {
  ...
  lexPostText(context, message) {
    ...
    if (message.data) {
      ...
      for (let i = 0; i < keys.length; i += 1) {
        session[keys[i]] = message.data[keys[i]];
      }
    }
    delete session.appContext;
    return context.dispatch('refreshAuthTokens')
      .then(() => context.dispatch('getCredentials'))
      .then(() => lexClient.postText(message.text, session))
      .then((data) => {
