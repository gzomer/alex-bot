        ...
        <v-layout column class="message-bubble-column">
          ...
          <v-flex d-flex class="message-bubble-avatar-container">
            <v-layout row class="message-bubble-row" 
              v-bind:class="{'message-bubble-row-image': 'image' in message}"
...
export default {
  ...
  computed: {
    ...
    imageBackground() {
      if ('image' in this.message) {
        return {
          'background-image': `url(${this.message.image})`,
...
.message-bot .message-bubble {
  background-color: #b3e5fc; /* red-50 from material palette */
}

.message-human .message-bubble {
  background-color: #e0e0e0; /* indigo-50 from material palette */
}
