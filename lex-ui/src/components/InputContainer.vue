...
export default {
  ...
  methods: {
    ...
    takePicture() {
      window.dispatchEvent(new MessageEvent('takePicture'));

      if (typeof this.listener === 'undefined') {
        this.listener = true;
        window.addEventListener('pictureData', this.uploadPicture, false);
      }
    },
    choosePicture() {
      window.dispatchEvent(new MessageEvent('choosePicture'));
