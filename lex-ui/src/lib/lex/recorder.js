...
export default class {
  ...
  init() {
    ...
    this._alreadyAddedAudioEvents = false;
    ...
    window.requestFileSystem(window.TEMPORARY, 5*1024*1024, function(fs) {
        ...
        window.audioinput.initialize(captureCfg, function() { 
            ...
            window.audioinput.checkMicrophonePermission(function(hasPermission) {
                if (hasPermission) {
                    console.info("Already have permission to record.");
                    startCapture();
                } 
                else {          
                    ...
                    window.audioinput.getMicrophonePermission(function(hasPermission, message) {
                        if (hasPermission) {
                            console.info("User granted permission to record.");
                            startCapture();
                        } else {
        ...
        console.warn("Couldn't access file system: " + e.message)
    });
  ...
  stop() {
    ...
    // Fix for FileReader not firing onloadend
    ...
    function onAudioInputFinish(evt) {
        // @ts-ignore
        window.resolveLocalFileSystemURL(evt.file, function (tempFile) {
          tempFile.file(function (tempWav) {
                var reader = new FileReader();    

                reader.onerror = function(e) {
                  console.warn('Open file error');
                }
            ...
            console.warn("Could not resolveLocalFileSystemURL: " + e.message);
        });
    }

    if (! window.attachedAudioEvents){
      window.attachedAudioEvents = true;
      window.addEventListener( "audioinputfinished", onAudioInputFinish, false );  
    }
    
    audioinput.stop(); // eslint-disable-line
    /*
